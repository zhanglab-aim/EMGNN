...
def set_masks(model: torch.nn.Module, mask: Tensor, edge_index: Tensor,
    ...
    for name,module in model.named_modules():
        if (isinstance(module, MessagePassing) and name.startswith("meta_gnn")):
...
class CaptumModel(torch.nn.Module):
    ...
    def forward(self, mask, *args):
        ...
        # Set edge mask:
        if self.mask_type == 'edge':
            set_masks(self.model, mask.squeeze(0), args[3],
                      apply_sigmoid=False)
        elif self.mask_type == 'node_and_edge':
            set_masks(self.model, args[0].squeeze(0), args[4],
                      apply_sigmoid=False)
            args = args[2:-1]
        ...
        if self.mask_type == 'edge':
            ...
            x = self.model(mask[0].squeeze(0),*args)
...
class Explainer(torch.nn.Module):
    ...
    def visualize_subgraph(self, node_idx: Optional[int], edge_index: Tensor,
                           ...
                           node_names: Optional[Tensor] = None,
        ...
        for i,(u,v) in enumerate(zip(edge_index[0],edge_index[1])):
            edge_mask_dict[(mapping[u.item()],mapping[v.item()])] = round(edge_mask[i].item(),2)

        nx.draw_networkx_edge_labels(G,pos,edge_mask_dict)

        if("labels" in label_kwargs):
            label_kwargs["labels"] = {k:v for k, v in label_kwargs["labels"].items() if k in pos}
